    ...
    <section id="hero" className="section-pad relative pt-28 sm:pt-32 lg:pt-36">
      <div className="container-shell grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        ...
        <div className="rounded-[2rem] bg-blue-900 border border-blue-700 p-6 text-white">
          <div className="mb-5 text-sm uppercase tracking-[0.25em] text-cyan-300/80">
            Contact & Presence
          </div>
          <div className="grid gap-4 text-sm text-white/90">
            <div className="flex items-center gap-3">
              <Phone className="h-4 w-4 text-cyan-300" /> {basics.phone}
            </div>
            <div className="flex items-center gap-3">
              <Mail className="h-4 w-4 text-cyan-300" /> {basics.email}
            </div>
            <div className="flex items-center gap-3">
              <MapPin className="h-4 w-4 text-cyan-300" /> {basics.location}
            </div>
            <a className="flex items-center gap-3 hover:text-white" href={basics.links[0].href}>
              <Linkedin className="h-4 w-4 text-cyan-300" /> {basics.links[0].value}
            </a>
            <a className="flex items-center gap-3 hover:text-white" href={basics.links[1].href}>
              <Github className="h-4 w-4 text-cyan-300" /> {basics.links[1].value}
            </a>
            <a className="flex items-center gap-3 hover:text-white" href={`https://${basics.website}`}>
              {basics.website}
            </a>
          </div>
        </div>
